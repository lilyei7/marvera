...
router.get('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    console.log(`📦 [GET /api/admin/products] ${products.length} productos encontrados`);
    res.json(products);
  } catch (error) {
...
// POST /api/admin/products - Crear nuevo producto (admin) - FIXED: images field
router.post('/', authenticateToken, upload.single('images'), async (req: AuthRequest, res) => {
  try {
    // Verificar que el usuario sea admin
    if (req.user?.role !== 'ADMIN') {
      return res.status(403).json({ message: 'Acceso denegado. Se requieren permisos de administrador.' });
    ...
    console.log('📝 [POST /api/admin/products] Creando producto:', { name, price, categoryId });
...
// PUT /api/admin/products/:id - Actualizar producto existente (admin) - FIXED: images field
router.put('/:id', authenticateToken, upload.single('images'), async (req: AuthRequest, res) => {
  try {
    // Verificar que el usuario sea admin
    if (req.user?.role !== 'ADMIN') {
      return res.status(403).json({ message: 'Acceso denegado. Se requieren permisos de administrador.' });
    ...
    console.log('📝 [PUT /api/admin/products] Actualizando producto:', id);
...
router.delete('/:id', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const { id } = req.params;
    console.log('🗑️ [DELETE /api/admin/products] Eliminando producto:', id);
