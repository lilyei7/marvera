...
app.get('/api/products/featured', async (req, res) => {
  try {
    ...
    // Transform for frontend with multiple images support
    const transformedProducts = featuredProducts.map(product => {
      let imageUrl = `/images/products/default-${product.category?.slug || 'product'}.jpg`;
      ...
      if (product.images) {
        try {
          ...
          if (Array.isArray(parsedImages) && parsedImages.length > 0) {
            imageUrl = parsedImages[0]; // Use first image
          } else if (typeof parsedImages === 'string') {
            imageUrl = parsedImages; // Single image string
          }
        } catch (e) {
          console.log('⚠️ Error parsing images for product:', product.name);
          imageUrl = product.images; // Use as is if not JSON
        }
      }
      ...
      return {
        ...
        imageUrl: imageUrl,
        images: product.images, // Include all images for frontend
...
app.post('/api/branches/upload-image', upload.single('image'), async (req, res) => {
  try {
    ...
    const imageUrl = `/uploads/branches/${req.files[0].filename}`;
...
app.post('/api/branches', upload.single('image'), async (req, res) => {
  try {
    ...
    const branchData = {
      ...
      imageUrl: req.files[0] ? `/uploads/branches/${req.files[0].filename}` : null
    };
...
app.put('/api/branches/:id', upload.single('image'), async (req, res) => {
  try {
    ...
    console.log('📸 New uploaded file:', req.files[0]);
    ...
    if (req.files[0]) {
      updateData.imageUrl = `/uploads/branches/${req.files[0].filename}`;
    }
...
// Endpoint de ofertas destacadas con mock data
app.get('/api/offers/featured', async (req, res) => {
  try {
    console.log('🌟 Featured offers requested');
